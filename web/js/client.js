...
function publish(address, message) {
	if (eb) {
		...
		$('#sent').append($("<code>").text("Address:" + address + " Message:"
				+ message));
		$('#sent').append($("</code><br>"));
	}
}
...
function subscribe(address) {
	if (eb) {
		eb.registerHandler(address, function(msg, replyTo) {
					$('#received').append("Address:" + address + " Message:"
							+ msg.text + "<br>");
				});
...
function openConn() {
	if (!eb) {
		eb = new vertx.EventBus("http://192.168.6.179:80/eventbus");
...
$(document).ready(function() {
			$("#sendButton").click(function() {
						publish($("#sendAddress").val(), $("#sendMessage")
										.val());
					});
